...
HANDLE_FRAME = 'refrigerator_frame'
...
def approach_ee_handle(env: ManagerBasedRLEnv, threshold: float) -> torch.Tensor:
    ...
    handle_pos = env.scene[HANDLE_FRAME].data.target_pos_w[..., 0, :] # TODO 
    # print("env.scene['cabinet'].data.target_pos_w", env.scene["cabinet_frame"].data.target_pos_w[..., 0, :])
...
def align_ee_handle(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    handle_quat = env.scene[HANDLE_FRAME].data.target_quat_w[..., 0, :]# TODO Frame 
...
def align_grasp_around_handle(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    handle_pos = env.scene[HANDLE_FRAME].data.target_pos_w[..., 0, :] # TODO FRAME 
...
def multi_stage_open_drawer(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    """
    'link_0 ?'
    print('the name of scene is ', env.scene.keys())
    print('------------', env.scene[asset_cfg.name].data.joint_pos.shape)
    print('!!!!!!!!!!!!!! ', asset_cfg)
    print('the data of scene is ', env.scene[asset_cfg.name].data.joint_pos)
    drawer_pos = env.scene[asset_cfg.name].data.joint_pos[:, asset_cfg.joint_ids[0]]
    is_graspable = align_grasp_around_handle(env).float()
    print('drawer_pos81 ', drawer_pos[81], drawer_pos.shape, asset_cfg.name)
    open_easy = (drawer_pos.abs() > 0.01) * 0.5
    open_medium = (drawer_pos.abs() > 0.2) * is_graspable
    open_hard = (drawer_pos.abs() > 0.3) * is_graspable
